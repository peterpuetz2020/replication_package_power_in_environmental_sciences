  ...
  freqs <- foreach(i = 1:length(summary(dat)[,1])) %dopar% { 
    ...
    for (j in 1:length(dat[[i]]$vi)) {
      ...
      for (a in 1:(length(z.grid)-1)) {
        ...
        freq[j,a] <- (pnorm(z.grid[a+1], mean = dat[[i]]$GE[j] / sqrt(dat[[i]]$vi[j]), sd = sqrt(dat[[i]]$vi[j]+0.25*dat[[i]]$tau2[j])/sqrt(dat[[i]]$vi[j])) - 
                        pnorm(z.grid[a], mean = dat[[i]]$GE[j] / sqrt(dat[[i]]$vi[j]), sd = sqrt(dat[[i]]$vi[j]+0.25*dat[[i]]$tau2[j])/sqrt(dat[[i]]$vi[j])))
      }
    }
    return(freq)
  }
