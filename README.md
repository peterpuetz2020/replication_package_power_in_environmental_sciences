...
Main-text Table 2 likewise has a fixed manuscript specification: a `0.5`
meta-average multiplier and `0`, `0.25`, `0.5`, and `0.75` heterogeneity
multipliers. The data-preparation stage always creates these four random-effects
inputs in addition to any custom setups, so Table 2 is rendered on custom runs.
...
This writes setup-specific derived datasets under `data/derived_data/`. `meta_average_multiplier` and `heterogeneity_multiplier` can each contain one or more values; `scripts/create_analysis_data.R` computes and stores outputs for every combination. To use custom labels, define an `analysis_setups` tibble/data frame with `meta_average_multiplier`, `heterogeneity_multiplier`, and `setup_label` columns before sourcing the script. The script also creates any missing setup-specific counterfactual z-value and p-value RDS files needed by the tables and plots. Cache metadata includes the input data, grid, setup, and (for confidence intervals) `n_iterations`; changing any of them replaces the stale file before rendering begins. The preparation and rendering scripts use the same cache format, so a freshly rebuilt result is reused rather than calculated a second time. Define `recreate_counterfactuals <- TRUE` before sourcing the script to overwrite and rebuild all matching counterfactual files even when that validation passes.
...
- `data/derived_data/regression/` contains generated ESR and regression-covariate RDS inputs.
